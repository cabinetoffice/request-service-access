...
describe('team endpoint integration tests', () => {

    const teamEndpoint = config.GITHUB_URL + config.CREATE + config.TEAM_URL;
    ...
    describe('GET tests', () => {
        test('renders the team page', async () => {
            const res = await request(app).get(teamEndpoint);

            expect(res.status).toEqual(200);
            expect(res.text).toContain(MOCK_GET_TEAM_RESPONSE);
    ...
    describe('POST tests', () => {
        ...
        test('Should render the same page with error messages after POST request', async () => {
            const res = await request(app).post(teamEndpoint).send({
                repo_name: '',
                github_handle: '',
            });
        ...
        test('Should log the team details POST request', async () => {
