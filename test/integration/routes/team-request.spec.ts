...
import { MOCK_REDIRECT_MESSAGE as MOCK_REDIRECT_MESSAGE, MOCK_GET_TEAM_REQUEST_RESPONSE, MOCK_POST_TEAM_REQUEST_RESPONSE } from '../../mock/text.mock';
...
describe('Team-request endpoint integration tests', () => {
    ...
    describe('GET tests', () => {
        test('renders the team-request page', async () => {
            const res = await request(app).get(config.TEAM_REQUEST_URL);
    ...
    describe('POST tests', () => {
        ...
        test('Should log the team name and description on POST request.', async () => {
            ...
            expect(mockLog).toBeCalledWith(MOCK_POST_TEAM_REQUEST_RESPONSE);
            expect(res.text).toContain(MOCK_REDIRECT_MESSAGE);
            expect(mockedLogger).toHaveBeenCalledTimes(1);
            expect(mockedAuth).toHaveBeenCalledTimes(1);
        });
